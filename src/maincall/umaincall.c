...
static void cross_call(umain_elf_t * _entry, umain_elf_t * _target, const char *name, struct umaincall * CallContext, uint64_t target)
{
        ...
        )
    {
        ...
        if (target >= openssl_area.text_begin && target <= openssl_area.text_end)
        {
            tmp.jmpcfg[idx_jmp++] = dasics_jumpcfg_alloc(openssl_area.text_begin, openssl_area.text_end);
            for(int i = 0; i < openssl_area.rw_num; i++)
            {
                tmp.handle[idx_lib++] = dasics_libcfg_alloc(openssl_area.rw_bound[i].flags, \
